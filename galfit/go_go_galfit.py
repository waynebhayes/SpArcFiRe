...
def check_update(log_out, galfit_num, components, max_it = 150, rerun = False):
...
def main(**kwargs):
    ...
    tmp_galfits = kwargs.get("tmp_galfits", pj(tmp_dir, "galfits"))
    tmp_png     = kwargs.get("tmp_png", pj(tmp_dir, "galfit_png"))
    out_png     = kwargs.get("out_png", pj(out_dir, "galfit_png"))
    
    # Of course the important things
    num_steps = int(kwargs.get("num_steps", 2))
    rerun     = bool(kwargs.get("rerun", False))
