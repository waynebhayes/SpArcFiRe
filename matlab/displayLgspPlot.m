function displayLgspPlot(params, bounds, img, ctrR, ctrC, barAngles, barHalfLens, lineColors, lineStyles, lineWidth, makeFig)
% Displays an image overlayed with plots of log-spiral arcs and bars
% Differs from displayLgspOverlay in that the result is a plot rather than
%   an image, making it easier to scale the arc display but adding
%   complications for saving the result to a file
% INPUTS:
...
if nargin < 6 || isempty(barAngles)
    barHalfLens = [];
end

if nargin < 7 || isempty(barHalfLens)
    barHalfLens = [];
end
...
if nargin < 9 || isempty(lineStyles)
    lineStyles = {'-'};
else
...
for arcIdx=1:1:size(params, 1);
    ...
    elseif params(arcIdx, 2) < 0
        ...
        lineType = ['m' lineStyles{arcIdx}];
    end

    [x, y] = logSpiralXY(params(arcIdx, :), bounds(arcIdx, :));
%     if isnumeric(lineColors)
