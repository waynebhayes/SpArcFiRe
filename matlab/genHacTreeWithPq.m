function [clusters, failed2revEncountered] = ...
    genHacTreeWithPq(simlMtx, img, stgs, tracePts, aviName, colorThres)
...
fprintf('%d cluster points with zero-valued intensities\n', ...
    sum(img(selPts) == 0))
...
toc; fprintf('...done initializing clusters\n');
...
while keepClustering
    ...
    if min(length(clus1.mergedPts), length(clus2.mergedPts)) > 25  %TODO: make a function of the image size!
        arcMergeChecks = arcMergeChecks + 1;
        mtxs = hac2mtxs([clus1 clus2], img, 0);
        [mergeErrRatio, anyFailed2rev] = ...
            calcArcMergeErr(mtxs(:, :, 1), [], [], [], ...
            mtxs(:, :, 2), [], [], [], ctrR, ctrC, [], stgs);
    ...
    if val >= stopThres      
        ...
        % TODO: take advantage of symmetry by using only a triangular part?
